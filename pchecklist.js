...
var passwordCheckOld = function () 
{
 ...
 if(stringPassword.length < 8) { //if statement to make sure the input is less than 8 characters
   stringOutput = "Password is not lengthy enough"; //output message
   $("output").value = stringOutput;
} else {
  for (var i = 0; i < arrayBadPasswords.length; i++) { //i is just a counter
      if(stringPassword == arrayBadPasswords[i])  { //checking to make sure the entered password is not included in the array
     stringOutput = "PASSWORD IS IN MY OMEGA AWESOME MASTER LIST OF BAD PASSWORDS TRY AGAIN!!!!";
     $("output").value = stringOutput;
     } else if(stringOutput == "") { //if the password is not in the array, output the good message
        stringOutput = "Your password is looking mighty fine, you should be alright";
     $("output").value = stringOutput;
